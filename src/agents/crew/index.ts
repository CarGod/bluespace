...
export function buildBrief(input: BuildBriefInput): string {
  ...
  const sections: string[] = [
    ...
    [
      ...
      '',
      'The directory outlives your run — nothing reclaims it while it still holds work that',
      'exists nowhere else. It becomes collectable only once the commits on your branch are',
      'merged into the base branch. Read that as one more reason to commit: uncommitted work',
      'is not a deliverable, it is a directory nobody can safely tidy up.',
      '',
...
function reconSection(reportPath: string, baseBranch: string): string {
  return [
    ...
    '   That file is archived out of this worktree when the task finishes, and the archived',
    '   copy is what the captain reads — so write it for someone who does not have this',
    '   directory, and never point at a path inside it as if it will still be there.',
